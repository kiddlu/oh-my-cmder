...
pause >nul
echo Playing audio...
%VLC% --demux rawaud --network-caching=50 tcp://localhost:%SNDCPY_PORT% --sout "#transcode{acodec=s16l,channels=2,ab=128,samplerate=44100}:std{access=file,mux=wav,dst=output.wav}" vlc://quit
goto :EOF
